boolean scan(float x, float y, float len_max)
{    //x,y coordinate del robot rispetto a SR0, len_max = raggio max laser
  ...
  if (is_in_obstacle(x, y) == detected_obs && (detected_obs != -1))
  {
    /* se il laser e l'oggetto si trovano lungo
     i lati dello stesso oggetto, same_obstacle è true quindi non viene utilizzato il laser altrimenti finirebbe dentro l'ostacolo*/
...
void detect_vert(float xi, float yi) //trova vertice -------------------------------------------------
{
  ...
  {
    // Se la x è uguale a quella precedente, la retta è verticale, diamo al coefficiente un valore grande
    m_i2_i1 = 1000;
  } else
  {
    // formula per calcolo coefficiente angolare
    m_i2_i1 = (y_prev[1]-y_prev[0])/(x_prev[1]-x_prev[0]);
  }

  if (x_prev[1] == xi)
  {
    // Se la x è uguale a quella del laser, la retta è verticale
    m_i1_i = 1000;
  } else
  {
  ...
  /* per trattare le coppie di punti a pendenza infinita */
