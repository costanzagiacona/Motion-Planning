...
float[] intersectionObstacles(float x, float y, float len_x, float len_y) //x,y posizione robot mentre lenx,leny sono le dimensioni del robot
{
  ...
  for (int i=0; i< ostacolo_ArrayList.size(); i++) //per ogni ostacolo
  {
    ...
    else if (o.forma == 4)
    {
      ...
      for (int ii = 0; ii < 10; ii=ii+4) //aumentando il numero dei vertici viene più preciso lo scan (6 vertici)
      // si arriva fino a 10 perché i vertici memorizzati sono 7, x0 = x7 e y0 = y7
      // prima arrivavamo a 8 e funzionava male
      {
...
/* Funzione che verifica se il primo punto (x1,y1) è più vicino al centro rispetto al secondo */
