...
boolean sovrapposizione(float posx, float posy, float l1,float l2, float alpha) //input - x ostacolo, y ostacolo, lati ostacolo, orientamento ostacolo
{
  ...
  //coordinate ostacolo rispetto SR0
    //vertice 1
    vert_ghost_obs[0] = (-l1/2-k)*cos(alpha) - (-l2/2-k)*sin(alpha)+posx;
  ...
  {
    if (abs(vert_ghost_obs[k]) > posxsp[nfigurasp-1]/1.45)  //1.45 scelto tramite ispezione visiva 
    {
      v1 = true;
      println("sovrapposizione TAVOLO x");
      return v1;
    }
    if (abs(vert_ghost_obs[k]) > posysp[nfigurasp-1]/1.45) 
    {
      v1 = true;
